...
class isInContourV1(Contour_Checking_fn):
	...
	def __call__(self, pt): 
		return 1 if cv2.pointPolygonTest(self.cont, _to_cv2_point(pt), False) >= 0 else 0
...
class isInContourV2(Contour_Checking_fn):
	...
	def __call__(self, pt): 
		...
		return 1 if cv2.pointPolygonTest(self.cont, _to_cv2_point(center), False) >= 0 else 0
...
class isInContourV3_Easy(Contour_Checking_fn):
	...
	def __call__(self, pt): 
		...
		for points in all_points:
			if cv2.pointPolygonTest(self.cont, _to_cv2_point(points), False) >= 0:
				return 1
		return 0
