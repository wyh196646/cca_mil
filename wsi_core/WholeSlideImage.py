...
class WholeSlideImage(object):
    ...
    def isInHoles(holes, pt, patch_size):
        center = (float(pt[0] + patch_size / 2), float(pt[1] + patch_size / 2))
        for hole in holes:
            if cv2.pointPolygonTest(hole, center, False) > 0:
                return 1
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        num_workers = int(os.environ.get('WSI_CONTOUR_WORKERS', min(mp.cpu_count(), 4)))
        num_workers = max(1, min(num_workers, mp.cpu_count()))
        if num_workers == 1:
            results = [
                WholeSlideImage.process_coord_candidate(*item)
